...
let
  ...
  attrs = finalAttrs: rec {
    pname = "redot4${suffix}";
    version = "4.3-stable";
    commitHash = "a06b0a021c7ed07e3c1fa45392fa0ab49453c395";
    ...
    installPhase =
      ''
        ...
        mkdir -p "$out/bin"
        cp bin/redot.* $out/bin/re dot4${suffix}

        installManPage misc/dist/linux/redot.6

        mkdir -p "$out"/share/{applications,icons/hicolor/scalable/apps}
        cp misc/dist/linux/org.godotengine.Redot.desktop "$out/share/applications/org.godotengine.Redot4${suffix}.desktop"
        substituteInPlace "$out/share/applications/org.godotengine.Redot4${suffix}.desktop" \
          --replace "Exec=redot" "Exec=$out/bin/redot4${suffix}" \
          --replace "Redot Engine" "Godot Engine 4"
        cp icon.svg "$out/share/icons/hicolor/scalable/apps/redot.svg"
        cp icon.png "$out/share/icons/redot.png"
      ''
      + lib.optionalString withMono ''
        cp -r bin/RedotSharp/ $out/bin/
        wrapProgram $out/bin/redot4${suffix} \
